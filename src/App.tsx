        ...
        >
          <div className={`p-6 border-b border-(--border-strong) flex items-center transition-all duration-300 ${isExpanded ? 'justify-between' : 'justify-center'}`}>
            ...
            <button 
              ...
              className="text-zinc-400 hover:text-white transition-colors shrink-0 focus:outline-none p-1 rounded-md hover:bg-[var(--bg-surface-hover)]"
        ...
        <main className="flex-1 overflow-y-auto relative">
          {!activeProject ? (
            <div className="flex flex-col items-center justify-center h-full text-center p-8 animate-in fade-in zoom-in duration-500">
              <div className="w-24 h-24 bg-[var(--bg-surface)] rounded-2xl flex items-center justify-center border border-[var(--border-strong)] mb-6 shadow-xl">
                <WebDesign01Icon size={48} className="text-[var(--accent-primary)]" />
              </div>
              <h2 className="text-3xl font-bold mb-3 tracking-tight">
                Welcome to <span style={{ fontFamily: '"Press Start 2P", system-ui' }}><span className="text-[var(--accent-primary)]">Ox</span>ide SEO</span>
              </h2>
              <p className="text-[var(--text-secondary)] max-w-md mb-8 text-lg">Your new centralized hub for local, privacy-first SEO tools. Please add your first website project using the folder icon in the sidebar to get started.</p>
            </div>
          ) : (
            <>
